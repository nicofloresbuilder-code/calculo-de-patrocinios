...
1. Crear el proyecto real de Supabase, correr `supabase/migrations/0001_init.sql`, habilitar Google como provider en Supabase Auth, y pegar `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` en `.env.local` + Vercel (Project Settings → Environment Variables, ahí también `SUPABASE_SERVICE_ROLE_KEY` si se termina necesitando) — para cerrar Commit 2 de verdad.
2. Conseguir `ANTHROPIC_API_KEY` y ponerla en `.env.local` + Vercel — para cerrar Commit 5 de verdad y hacer el deploy 2.
3. Con 2 y 5 reales: cerrar Commit 6 (guardar + RLS con dos usuarios distintos) y Commit 7 (pase mecánico con los 3 deals reales, corrigiendo el clamp de aforo y la base de `oficial` que ya se ven sobre/subestimados).
